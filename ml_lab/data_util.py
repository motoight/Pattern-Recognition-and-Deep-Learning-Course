...
def pre_data():
    # 数据准备
    train_X_path = 'dataset/MNIST/train-images.idx3-ubyte'
    train_Y_path = 'dataset/MNIST/train-labels.idx1-ubyte'
    test_X_path = 'dataset/MNIST/t10k-images.idx3-ubyte'
    test_Y_path = 'dataset/MNIST/t10k-labels.idx1-ubyte'

    # 获取训练数据和测试数据
    train_x = DataUtils(train_X_path).getImage()
    train_y = DataUtils(train_Y_path).getLabel()

    test_x = DataUtils(test_X_path).getImage()
    test_y = DataUtils(test_Y_path).getLabel()

    np.savez('train_data', train_x, train_y)
    np.savez('test_data', test_x, test_y)

def load_data():
    train = np.load('train_data.npz')
    test = np.load('test_data.npz')
    # load npz 需要按照默认的key 索引
    return train['arr_0'], train['arr_1'], test['arr_0'], test['arr_1']